...
export default function HomePage() {
  ...
  className={`w-48 h-36 shrink-0 snap-start bg-(--loom-cyan)/20 p-4 rounded-xl cursor-pointer relative transition-all duration-300 try-it-card flex flex-col items-center justify-center text-center ${
    isActive || isClicked ? 'snake-active' : ''
  }`}
>
  <div className="flex flex-col items-center gap-1">
    {quiz.category?.iconUrl ? (
      <img src={quiz.category.iconUrl} alt="" className="w-8 h-8 rounded object-contain bg-(--loom-black)/40 p-1" />
    ) : (
      <div className="w-8 h-8 rounded bg-(--loom-black)/40 flex items-center justify-center text-(--loom-white) text-xs font-bold">
