...
>
  ...
  <div className="flex items-center justify-between mb-1.5">
    <div className="flex items-center gap-2">
      {quiz.category?.iconUrl ? (
        <img src={quiz.category.iconUrl} alt="" className="w-5 h-5 rounded-full object-contain" />
      ) : (
        <div className="w-5 h-5 rounded-full bg-(--loom-cyan)/20 flex items-center justify-center text-(--loom-cyan) text-[10px] font-bold">
          {quiz.category?.name?.[0] || '?'}
        </div>
      )}
      <span className="text-xs text-(--loom-white)/40">•</span>
    </div>
    {quiz.level && (
      <span
        className={cn(
  ...
  {/* Название */}
  <h3 className="font-bold text-lg text-(--loom-white) truncate leading-tight mb-1">{quiz.title}</h3>

  {/* Описание (заполняет низ) */}
  <p className="flex-1 text-sm text-(--loom-white)/60 pb-2">{quiz.description}</p>
</div>
                  );
                })
              )}
