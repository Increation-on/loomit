...
>
  <div className="flex items-center justify-center mb-2">
    {quiz.category?.iconUrl ? (
      <img src={quiz.category.iconUrl} alt="" className="w-8 h-8 rounded-full bg-(--loom-white)/10 p-1 object-contain" />
    ) : (
      <div className="w-8 h-8 rounded-full bg-(--loom-white)/10 flex items-center justify-center text-(--loom-cyan) font-bold">
        {quiz.category?.name?.[0] || '?'}
      </div>
    )}
  </div>
  <h3 className="font-bold text-lg text-(--loom-white) truncate leading-tight">{quiz.title}</h3>
  <p className="flex-1 text-sm text-(--loom-white)/60 line-clamp-2 pb-2">{quiz.description}</p>
</div>
                  );
                })
              )}
