                ...
                shuffledQuizzes.map((quiz: any, index: number) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-48 h-36 shrink-0 snap-start bg-(--loom-cyan)/20 p-4 rounded-xl cursor-pointer relative transition-all duration-300 try-it-card flex flex-col ${isActive || isClicked ? 'snake-active' : ''
                        }`}
                    >
                      <div className="flex items-center justify-center -mt-1 mb-0.5">
                        {quiz.category?.iconUrl ? (
                          <img src={quiz.category.iconUrl} alt="" className="w-7 h-7 rounded object-contain bg-(--loom-black)/40 p-0.5" />
                        ) : (
                          <div className="w-7 h-7 rounded bg-(--loom-black)/40 flex items-center justify-center text-(--loom-white) text-xs font-bold mt-1">
                            {quiz.category?.name?.[0] || '?'}
                          </div>
                        )}
                      </div>
                      <h3 className="font-bold text-lg text-(--loom-white) truncate leading-tight">{quiz.title}</h3>
